...
console.clear();
console.log(
  chalk.green(figlet.textSync("Health Assistant", { horizontalLayout: "full" }))
);

// Custom print function to wrap text and display in a box
function customPrint(text = "", options = {}) {
  const {
    wrap = 80,
    padding = 1,
    margin = 1,
  ...
  const wrappedText = wrapAnsi(text, wrap);
  const boxOptions = {
    padding,
    margin,
    borderColor,
    borderStyle,
    title: title ? chalk.green(title) : "",
  };

  console.log(boxen(chalk.green(dedent`${wrappedText}`), boxOptions));
}

function customPrint2(text = "", options = {}) {
  const {
    wrap = 80,
    padding = 1,
    margin = 1,
  ...
  const boxOptions = {
    ...
    borderColor,
    borderStyle,
    title: title ? chalk.red(title) : "",
  };

  console.log(boxen(chalk.red(dedent`${wrappedText}`), boxOptions));
}

function customPrint3(text = "", options = {}) {
  const { wrap = 80 } = options;

  const wrappedText = wrapAnsi(text, wrap);

  const styledText = chalk.cyan(dedent`${wrappedText}`);
...
//Main Function
async function main() {
  // Example usage of print for initial message
  customPrint(
  ...
  async function generateQuestion(currentContext, userResponse) {
    ...
    const nextQuestionResponse = await gptPrompt(
      prompt,
      {
        max_tokens: 100,
        temperature: 0.7,
  ...
  if (reportedDiscomfort.length > 0) {
    ...
    customPrint(reportedDiscomfortString, {
      title: "💭Here are the advices for your discomforts",
    });
    ...
    userResponse = await ask(customPrint3(tcmInterestQuestionString));
    ...
    if (userResponse.toLowerCase() === "yes") {
      ...
      const tcmAdvice = await gptPrompt(
        tcmAdvicePrompt,
        {
          max_tokens: 1024,
          temperature: 0.5,
      ...
      const tcmAdvice2 = await gptPrompt(
        massageInstructions,
        {
          max_tokens: 50,
          temperature: 0.5,
      ...
      customPrint(tcmAdvice2, {
        title: "💭Massage Instructions",
        borderColor: "green",
        borderStyle: "round",
      });
      ...
      const tcmAdvice3 = await gptPrompt(
        Caution,
        {
          max_tokens: 50,
          temperature: 0.5,
      ...
      customPrint2(tcmAdvice3, {
        title: "💭Caution",
      });

      // Tuina Plan
      const tuinaPlanQuestionPrompt = `Directly asking if the user is interested in receiving Tuina Plan in 10 words.Do not say you are an AI.`;
      const tuinaPlanQuestionString = await gptPrompt(
        tuinaPlanQuestionPrompt,
        {
          max_tokens: 40,
          temperature: 0.5,
        },
      ...
      userResponse = await ask(customPrint3(tuinaPlanQuestionString));
      ...
      if (userResponse.toLowerCase() === "yes") {
        ...
        const tcmAdvice4 = await gptPrompt(
          tuinaPlan,
          {
            max_tokens: 1000,
            temperature: 0.5,
