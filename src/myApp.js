...
async function main() {
  ...
  if (reportedDiscomfort.length > 0) {
    ...
    const tcmInterestQuestionPrompt = `Generate a polite and engaging question directly asking if the user is interested in receiving further Traditional Chinese Massage (TCM) advice based on their discomfort and intruduce Traditional Chinses Massage briefly in 40 words paragraph.`;
    ...
    if (interestResponse.toLowerCase() === "yes") {
      const tcmAdvicePrompt = `Based on the discomforts reported: ${discomfortsForPrompt}, provide Traditional Chinese Massage point advice for each discomfort. Include specific body points, their information, and function. Do not say you are an AI.`;
